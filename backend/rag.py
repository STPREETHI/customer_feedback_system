...
print("--- Initializing RAG models (T5-small)... ---")
...
def analyze_reviews_definitively(reviews: list):
    ...
    adv_prompt = f"Based on these reviews, list three key advantages in a bulleted list. Use proper capitalization. Reviews: \"{combined_reviews_text}\""
    advantages_text = generate_with_t5(adv_prompt, max_length=100)
    advantages = [adv[1] for adv in re.findall(r'(\d+\.\s*|-\s*|\*\s*)([^\n]+)', advantages_text)]

    dis_prompt = f"Based on these reviews, list three key disadvantages in a bulleted list. Use proper capitalization. Reviews: \"{combined_reviews_text}\""
...
def summarize_and_analyze_comparison(analysis1: dict, analysis2: dict):
    """Generates a detailed comparison summary using the new rich data."""
    prompt = f"""
